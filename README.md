...
First, you'll have to obtain credentials from Google that will allow ForTheRecord to use the Gmail API. The process is very similar to that of [gog](https://gogcli.sh/quickstart.html#2-get-an-oauth-client), the Google CLI that has become so fashionable among OpenClaw users:
